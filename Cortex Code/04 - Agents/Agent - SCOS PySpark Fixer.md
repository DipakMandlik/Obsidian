...
name: scos-pyspark-fixer
title: "SCOS PySpark Fixer"
source_repo: Snowflake-Labs/coco-skills
source_path: skills/spark-migration/snowpark-connect/migrate-pyspark-to-snowpark-connect/agents/fixer.md
source_url: https://github.com/Snowflake-Labs/coco-skills/blob/28b549f48da9994307081a9d4d2f16379f5a9c16/skills/spark-migration/snowpark-connect/migrate-pyspark-to-snowpark-connect/agents/fixer.md
...
**Type:** Cortex Code Agent
**Agent Name:** `scos-pyspark-fixer`
**Source:** Snowflake-Labs/coco-skills
**Repository Path:** `skills/spark-migration/snowpark-connect/migrate-pyspark-to-snowpark-connect/agents/fixer.md`
**Source URL:** [skills/spark-migration/snowpark-connect/migrate-pyspark-to-snowpark-connect/agents/fixer.md](https://github.com/Snowflake-Labs/coco-skills/blob/28b549f48da9994307081a9d4d2f16379f5a9c16/skills/spark-migration/snowpark-connect/migrate-pyspark-to-snowpark-connect/agents/fixer.md)
**Status:** Available
...
Execute specialized sub-phase operations for SCOS PySpark Fixer in the Snowpark Connect (SCOS) migration and validation pipeline.

---

## Responsibilities

- Perform deterministic execution for SCOS PySpark Fixer
...
```text
Spawn subagent scos-pyspark-fixer with role instructions and target parameters.
```
...
3. For `# SCOS-WARN:` and `# SCOS-TODO:` comments (annotate-only recipes):
   ...
   - `nested_window_functions_annotate`: split a nested
     `lag`/`lead`/`last`/`last_value`/`first`/`first_value` (outer
     function whose args contain another `.over(...)`) into
     `withColumn` steps — materialize the inner window, then apply
     the same outer function.
   - `create_map_array_keys_annotate`: do not emit ARRAY-keyed maps;
     rewrite `create_map(collect_list, collect_list)` to
     `map_from_arrays`, a join, or stringify keys.
...
4. For each remaining issue, consult `references/fix-rules.md` for the appropriate action:
   ...
   - **Nested-lambda variable capture (`Cannot resolve variable '<v>' within lambda function`)**: Read `../../references/python/nested-lambda-capture.md` before rewriting one — the fix pairs the captured value into the inner collection and reads it off the inner lambda's own parameter, but for `filter` / `array_sort` / `map_filter` / `transform_keys` the pairing alone silently changes the output type and needs the unwrap in that file's table, and the `(element, index)` overload of `F.transform` / `F.filter` has no rewrite at all (annotate instead)
   - **`array_sort` 2-arg comparator (`Unsupported function name array_sort`, `SPRKCNTPY5200`)**: 1-arg `array_sort` is fine; the comparator overload is unmapped. Three paths only (Rule 33): `f(left)-f(right)` → decorate-sort-undecorate (pack multi-level keys into one scalar — SCOS sorts `ARRAY<STRUCT>` by field name, not position); an `isNull`-gated 0 is a stability freeze (nulls-last + caveat, not a key); anything else (e.g. `when(x == sentinel_val, 1).otherwise(x < y)`) → `# SCOS: TODO` with why it is not reducible. Do not apply this to `zip_with`.
...
- **Repository File:** `skills/spark-migration/snowpark-connect/migrate-pyspark-to-snowpark-connect/agents/fixer.md`
- **GitHub URL:** [Snowflake-Labs/coco-skills/skills/spark-migration/snowpark-connect/migrate-pyspark-to-snowpark-connect/agents/fixer.md](https://github.com/Snowflake-Labs/coco-skills/blob/28b549f48da9994307081a9d4d2f16379f5a9c16/skills/spark-migration/snowpark-connect/migrate-pyspark-to-snowpark-connect/agents/fixer.md)
